<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>R&M Engenharia</title>
    <link rel="icon" type="image/png" href="src/img/logoeng.png">
    <!-- ROTA -->
    <base href="/">
    <!-- Principal CSS -->
    <link rel="stylesheet" href="principalstyle.css" />

    <!-- Customized CSS -->
    <link rel="stylesheet" href="src/css/especificidade-css.css" />
    <!-- Icons Awesome -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">

    <!-- Bootstrap CSS -->

    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
</head>

<body>

<?php include $_SERVER['DOCUMENT_ROOT'] . '/php/header.php'; ?>
    <main class="root">
        <section class="main-class">
            <div class="image-controller">
                <img class="img-fluid image-fundo" #id="engbannerfront" src="src/img/engbannerfront.png" alt="Imagem de fundo">

            </div>
            </div>
            <div class="image-controller">
                <img class="img-fluid image-fundo" #id="engbannerback" src="src/img/engbannerback.png" alt="Imagem de fundo">

            </div>

            <div class="container-xxl hidden my-6 py-6 pt-0" style="margin-top:2.5rem;">
                <div class="container">
                    <div class=" mx-auto mb-5">
                        <div class="image-controller hidden">
                            <img class="image-fundo-mobile" src="src/img/solocoes-engenharia.png" alt="Imagem de fundo">

                        </div>
                    </div>
                </div>
            </div>
            <div class="container-xxl my-6 py-6 pt-0 hidden" style="background-color:transparent;margin-top:2.5rem;">
                <div class="container">
                    <div class=" mx-auto mb-5">
                        <!-- <h2 class="display-6 mb-2" style="color:#55A5A5">R&M Engenharia</h2> -->
                        <p class="light-text">
                        O setor de engenharia do Grupo R&M atende construtoras e condomínios que buscam os seguintes serviços:<br>
                        </p>
                        
                    </div>
                    <div class="row g-4" style="justify-content: center;">
                        <div class="col-lg-4 col-md-6">
                            <div class="product-item d-flex flex-column rounded overflow-hidden text-center h-100">
                                <div class="text-center p-4">
                                    <svg width="50px" height="50px" viewBox="0 0 64 64" id="Layer_1_1_" version="1.1" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">

                                        <g>

                                            <g>

                                                <path d="M53,33c-3.309,0-6-2.691-6-6V16c0-0.72-0.064-1.482-0.197-2.331H17.197    C17.064,14.518,17,15.28,17,16v11c0,3.309-2.691,6-6,6H9.132l2.263,3.395c1.501,2.251,3.794,3.78,6.375,4.349L26,38v-4h0.018    c-1.926-1.443-3.321-3.555-3.817-6H22c-2.209,0-4-1.791-4-4c0-2.209,1.791-4,4-4v-1.757c-1.872-1.256-3-2.785-3-4.243h26    c0,1.458-1.128,2.987-3,4.243V20c2.209,0,4,1.791,4,4c0,2.209-1.791,4-4,4h-0.201c-0.496,2.445-1.891,4.557-3.817,6H38v4    l8.23,2.743c2.581-0.569,4.874-2.098,6.375-4.349L54.868,33H53z" style="fill:#00F4CB;" />

                                            </g>

                                            <g>

                                                <rect height="6" style="fill:#00F4CB;" width="10" x="35" y="50" />

                                            </g>

                                            <g>

                                                <rect height="6" style="fill:#00F4CB;" width="10" x="19" y="50" />

                                            </g>

                                            <g>

                                                <circle cx="27" cy="23" r="1" />

                                                <circle cx="37" cy="23" r="1" />

                                                <path d="M32,32c2.206,0,4-1.794,4-4h-2c0,1.103-0.897,2-2,2s-2-0.897-2-2h-2C28,30.206,29.794,32,32,32z" />

                                                <path d="M49.214,40.685L39,37.279v-2.801c1.711-1.415,2.978-3.341,3.589-5.538C45.066,28.645,47,26.555,47,24    c0-2.414-1.721-4.434-4-4.899v-0.354c1.547-1.136,2.601-2.48,2.904-3.902C47.115,14.459,48,13.337,48,12    c0-1.567-1.212-2.843-2.746-2.974c-2.336-4.424-6.756-7.402-11.681-7.932C33.391,1.039,33.201,1,33,1h-2    c-0.201,0-0.391,0.039-0.573,0.094c-4.924,0.529-9.345,3.507-11.681,7.932C17.212,9.157,16,10.433,16,12    c0,1.337,0.885,2.459,2.096,2.846c0.303,1.421,1.357,2.766,2.904,3.902v0.354c-2.279,0.465-4,2.484-4,4.899    c0,2.555,1.934,4.645,4.411,4.94c0.611,2.197,1.878,4.123,3.589,5.538v2.801l-10.214,3.405C11.924,41.638,10,44.307,10,47.324V63    h44V47.324C54,44.307,52.076,41.638,49.214,40.685z M52,47.324V56h-6v-4.053c0-1.185,0.19-2.355,0.564-3.479l1.968-5.904    l0.049,0.016C50.626,43.263,52,45.169,52,47.324z M19.332,47.836l-1.968-5.903l5.129-1.71l2.967,5.935l-0.015,0.01l0.082,0.123    L28,51.236V61h-8v-9.053C20,50.547,19.775,49.164,19.332,47.836z M28.039,46.842l3.894-2.596L32,44.302l0.067-0.056l3.894,2.596    L34,50.764V61h-4V50.764L28.039,46.842z M36,51.236l2.473-4.945l0.082-0.123l-0.015-0.01l2.967-5.935l5.129,1.71l-1.968,5.903    C44.225,49.164,44,50.547,44,51.947V61h-8V51.236z M39.59,39.584l-2.728,5.456l-3.193-2.129l4.544-3.786L39.59,39.584z M20.257,15    h23.485c-1.101,2.294-5.694,5-11.743,5S21.359,17.294,20.257,15z M45,24c0,1.317-0.859,2.427-2.042,2.829    C42.979,26.554,43,26.28,43,26v-4.816C44.161,21.598,45,22.698,45,24z M29,3.364V7h2V3.042C31.331,3.016,31.665,3,32,3    s0.669,0.016,1,0.042V7h2V3.364C38.242,4.145,41.106,6.163,42.929,9H21.071C22.894,6.163,25.758,4.145,29,3.364z M19,11h26    c0.552,0,1,0.448,1,1s-0.448,1-1,1H19c-0.552,0-1-0.448-1-1S18.448,11,19,11z M19,24c0-1.302,0.839-2.402,2-2.816V26    c0,0.28,0.021,0.554,0.042,0.829C19.859,26.427,19,25.317,19,24z M23,26v-6.039C25.493,21.21,28.675,22,32,22s6.507-0.79,9-2.039    V26c0,4.963-4.037,9-9,9S23,30.963,23,26z M32,37c1.801,0,3.498-0.444,5-1.214v1.745l-5,4.167l-5-4.167v-1.745    C28.502,36.556,30.199,37,32,37z M25.787,39.125l4.544,3.786l-3.193,2.129l-2.728-5.456L25.787,39.125z M15.419,42.581    l0.049-0.016l1.968,5.904C17.81,49.592,18,50.763,18,51.947V56h-6v-8.676C12,45.169,13.374,43.263,15.419,42.581z M12,58h6v3h-6    V58z M46,61v-3h6v3H46z" />

                                            </g>

                                            <ol class="text-start">
                                                <li><span class="bullet-eng"></span> Laudos: apontamento técnico e emissão de relatórios</li>
                                                <li><span class="bullet-eng"></span> Relatórios de vistorias</li>
                                                <li><span class="bullet-eng"></span> Emissão de ART para reformas</li>
                                                <li><span class="bullet-eng"></span> Acompanhamento de reforma</li>
                                            </ol>
                                </div>
                            </div>
                        </div>

        </section>


        <?php include $_SERVER['DOCUMENT_ROOT'] . '/php/whatsapp.php'; ?>
    </main>
    
    <?php include $_SERVER['DOCUMENT_ROOT'] . '/php/footer.php'; ?>
    <script src="src/js/jquery-3.3.1.min.js"></script>
    <script src="src/js/popper.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>

</body>

</html>